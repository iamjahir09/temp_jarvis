...
def playAssistantSound():
    music_dir = "Public-CHARVIS\\www\\assets\\audio\\start_sound.mp3"
    playsound(music_dir)
...
def hotword():
    ...
    try:
        ...
        while True:
            ...
            # Process the audio data to detect the hotword
            ...
            if keyword_index >= 0:
                ...
                # Activate Charvis (Replace this with actual activation logic)
                activate_charvis()
...
def activate_charvis():
    # Your logic to activate Charvis goes here
...
def searchGoogle(query):
    ...
    if search_query:
        ...
        try:
            ...
            autogui.hotkey('ctrl', 'l')  # Focus on the address bar
            autogui.typewrite(f"https://www.google.com/search?q={quote(search_query)}")
            autogui.press('enter')  # Execute the search
...
from engine.command import detect_tone

def charvis_response(user_input):
    tone = detect_tone(user_input)
    
    if tone == "positive":
        response = "I'm glad you're feeling good! How can I assist you further?"
    elif tone == "negative":
        response = "It seems like you're having a tough time. How can I help?"
    else:
